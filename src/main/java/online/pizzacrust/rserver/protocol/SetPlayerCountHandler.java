...
public class SetPlayerCountHandler extends AbstractHandler {
    @Override
    public void handle(String s, Request implRequest, HttpServletRequest apiRequest,
        ...
        if (!key.contains("code") || !key.contains("instance_id") || !key.contains("number")) {
        ...
        if (registrationsResponse[0] != null) {
            registrationsResponse[0].registeredPlayerCount = registrationsResponse[0]
                    .registeredPlayerCount + Integer.parseInt(apiRequest.getParameter("number"));
        }
